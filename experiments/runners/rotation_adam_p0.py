...
from experiments.core.plotting import plot_error_band, plot_matrix_heatmap, plot_multi_curve, write_figure_index
...
class RotationAdamP0Runner(BaseExperimentRunner):
    """Runner for the `P = 0` constrained Adam experiment.
    ...
    def train_stage(self) -> Dict[str, Any]:
        """Train all seeded models for the constrained Adam setup.
    ...
    def eval_stage(self, train_artifacts: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluate all checkpoints for the constrained Adam setup.
        ...
        for seed_key, payload in train_artifacts["seed_artifacts"].items():
            ...
            rotated_distance_curves[seed_key] = compute_distance_curves(
                history = history,
            ...
            raw_distance_curves[seed_key] = compute_distance_curves(
                history = history,
                n_layer = self.config.n_layer,
                include_p = False,
                rotate_q = False
        ...
        return {
            ...
            "rotated_distance_curves": rotated_distance_curves,
            "rotated_distance_summary": summarize_component_curves(rotated_distance_curves),
            "raw_distance_curves": raw_distance_curves,
            "raw_distance_summary": summarize_component_curves(raw_distance_curves),
        }

    def plot_stage(self, train_artifacts: Dict[str, Any], metrics: Dict[str, Any]) -> None:
        """Render all paper-style figures for the constrained Adam experiment.
        ...
        loss_paths = plot_error_band(
            ...
            x_label = "Iteration",
            y_label = "log(Loss)",
            basename = "rotation_demonstration_adam_pnull_loss_plot",
        ...
        figure_index.append({
            "experiment": self.config.experiment,
            "basename": "rotation_demonstration_adam_pnull_loss_plot",
            "paper_use": "Log loss curve for the constrained Adam setting",
            "paths": loss_paths,
        })

        for component_name in ["A0", "A1", "A2"]:
            component_paths = plot_multi_curve(
                x_values = iterations,
                curves = [
                    {
                ...
                basename = f"rotation_demonstration_dist_to_id_adam_pnull_{component_name}",
                output_dir = self.paths.figures_dir,
                dpi = self.config.line_dpi,
                figsize = (9, 7)
            )
            figure_index.append({
                "experiment": self.config.experiment,
                "basename": f"rotation_demonstration_dist_to_id_adam_pnull_{component_name}",
                "paper_use": f"Dual distance curve for {component_name}",
        ...
        for matrix_name, matrix in matrices.items():
            heatmap_paths = plot_matrix_heatmap(
                matrix = matrix,
                title = f"$\\Sigma^{{1/2}} {matrix_name} \\Sigma^{{1/2}}$",
                basename = f"rotation_demonstration_pnull_{matrix_name}",
                output_dir = self.paths.figures_dir,
            ...
            figure_index.append({
                "experiment": self.config.experiment,
                "basename": f"rotation_demonstration_pnull_{matrix_name}",
                "paper_use": f"Final rotated heatmap for {matrix_name}",
                "paths": heatmap_paths,
            })
