...
class RotationLbfgsRunner(BaseExperimentRunner):
    """Runner for the rotated-covariance LBFGS experiment.
    ...
    def train_stage(self) -> Dict[str, Any]:
        """Train all seeded models for the LBFGS setup.
        ...
        for seed in self.config.seeds:
            ...
            if "train_snapshot" in seed_payload:
                save_data_asset(self.paths.data_root / f"{seed_key}_train_snapshot.pt", seed_payload["train_snapshot"])
        return {"seed_artifacts": seed_artifacts}

    def eval_stage(self, train_artifacts: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluate all checkpoints for the LBFGS setup.
    ...
    def plot_stage(self, train_artifacts: Dict[str, Any], metrics: Dict[str, Any]) -> None:
        """Render all paper-style figures for the LBFGS experiment.
        ...
        loss_paths = plot_error_band(
            ...
            basename = "rotation_demonstration_loss_plot",
        ...
        figure_index.append({
            "experiment": self.config.experiment,
            "basename": "rotation_demonstration_loss_plot",
            "paper_use": "Loss curve for the LBFGS rotation experiment",
            "paths": loss_paths,
        })
        ...
        panel_paths = plot_distance_panels(
            ...
            basename = "rotation_demonstration_dist_to_id_lbfgs",
            output_dir = self.paths.figures_dir,
            dpi = self.config.line_dpi
        )
        figure_index.append({
            "experiment": self.config.experiment,
            "basename": "rotation_demonstration_dist_to_id_lbfgs",
            "paper_use": "Combined distance panel for the LBFGS rotation experiment",
            "paths": panel_paths,
        })
        ...
        for matrix_name, matrix in matrices.items():
            heatmap_paths = plot_matrix_heatmap(
                ...
                basename = f"rotation_demonstration_{matrix_name}",
                output_dir = self.paths.figures_dir,
